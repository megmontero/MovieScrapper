...
class IMDBMovieExtractor():
    ...
    def get_movie_info(self, movie_page, movie_rating_page,
        ...
        movie_info = json.loads(soup.find('script',
                                          type='application/ld+json').text)
        titleyear = (soup.select('h1')[0].text.strip()).split("\xa0")
        # titleyear[0] Spanish title
        movie_info["title"] = movie_info["name"]
        if len(titleyear) > 1:
        ...
        if "actor" in movie_info:
            movie_info["actors"] = \
                self.extract_person_list(movie_info["actor"])
        if "creator" in movie_info:
            movie_info["creators"] = \
                self.extract_person_list(movie_info["creator"])
        if "director" in movie_info:
            movie_info["directors"] = \
                self.extract_person_list(movie_info["director"])
        ...
        movie_info["reviews"] = \
            self._get_movie_reviews_info(movie_reviews_page)
    ...
    def _get_movie_rating_info(self, movie_rating_page):
        ...
        for c, label in enumerate(demo_labels):
            try:
                rating = float(rating_cells[c].find("div",
                                                    {"class": "bigcell"})
                               .text.strip())
                votes = int(rating_cells[c].find("div",
                                                 {"class": "smallcell"})
                            .text.replace(",", "")
                            .strip())
            except Exception:
    ...
    def get_person_info(self, person_page):
        ...
        casting_section = soup.findAll("div",
                                       {"class": "filmo-category-section"})
        script_person_info = json.loads(soup.find('script',
                                                  type='application/ld+json')
                                        .text)
        person_info = {}
        person_info["id"] = soup.find("meta",
                                      property="pageId")["content"]
        ...
        for category, section in zip(casting_categories, casting_section):
            ...
            item_list = []
            section_casting_odd = section.findAll("div",
                                                  {"class": "filmo-row odd"})
            section_casting_even = section.findAll("div",
                                                   {"class": "filmo-row even"})
    ...
    def get_user_ratings(self, user_page):
        ...
        for it in items:
            ...
            rate = {"global": float(starts[0].text),
                    ...
                    "movie": {"id": re.search("tt\d{7}", movie["href"])
                              .group(0),
                              "title": movie.text}}
